...
<table id="customers">
        ...
        try{
        	...
        	ResultSet rs = st.executeQuery("select * from cart inner join product on cart.price=product.price and cart.orderdate is not NULL and cart.status='processing'");
        	while(rs.next()){
        %>
             ...
             <td><%=rs.getString(11) %></td>
              <td><%=rs.getString(12) %></td>
               ...
               <td><a href="cancelorderaction.jsp?id=<%=rs.getString(2)%>&email=<%=rs.getString(1)%>">Cancel <i class='fas fa-window-close'></i></a></td>
                <td><a href="deliverorderaction.jsp?id=<%=rs.getString(2)%>&email=<%=rs.getString(1 )%>">Deliver <i class='fas fa-dolly'></i></i></a></td>
            </tr>
         <%}
        }catch(Exception e){
        	e.printStackTrace();
